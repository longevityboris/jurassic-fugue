...
usage: python3 check.py [--bars A-B] [--grid] [--quiet]
...
SRC = open(sys.argv[1] if len(sys.argv) > 1 and sys.argv[1].endswith('.ly')
           else 'fugue.ly').read()
VOICES = ['soprano', 'alto', 'tenor', 'pedal']
ABBR = {'soprano': 'S', 'alto': 'A', 'tenor': 'T', 'pedal': 'P'}
RANGE = {'soprano': (36, 84), 'alto': (36, 84), 'tenor': (36, 84), 'pedal': (36, 62)}
...
def parse(voice):
    m = re.search(voice + r"\s*=\s*\\absolute\s*\{(.*?)^\}", SRC, re.S | re.M)
    ...
    while pos < len(body):
        ...
        if g['note']:
            ...
            if t != barno:
                ...
                barno = int(t) if t.denominator == 1 else barno
            barno += 1
    return notes, t, errs
