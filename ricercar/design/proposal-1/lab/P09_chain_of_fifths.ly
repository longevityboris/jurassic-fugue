\version "2.24.0"
%% proposal-1 lab: P09 chain bes-ees-aes-Des, 2 bars apart (bars 27-37)
\header { title = "P09 chain bes-ees-aes-Des, 2 bars apart (bars 27-37)" tagline = ##f }
global = { \key bes \minor \time 2/2 }

soprano = \absolute {
  r1 |
  r1 |
  r1 |
  r1 |
  r1 |
  r1 |
  des''2. des''8 c''8 |
  des''2. des''8 c''8 |
  des''4. ees''8 ees''4. ges''8 |
  ges''2. f''8 des''8 |
  ees''2 r2 |
}

alto = \absolute {
  bes'2. bes'8 a'8 |
  bes'2. bes'8 a'8 |
  bes'4. c''8 c''4. ees''8 |
  ees''2. des''8 bes'8 |
  c''2 r2 |
  r1 |
  r1 |
  r1 |
  r1 |
  r1 |
  r1 |
}

tenor = \absolute {
  r1 |
  r1 |
  ees'2. ees'8 d'8 |
  ees'2. ees'8 d'8 |
  ees'4. f'8 f'4. aes'8 |
  aes'2. ges'8 ees'8 |
  f'2 r2 |
  r1 |
  r1 |
  r1 |
  r1 |
}

bass = \absolute {
  r1 |
  r1 |
  r1 |
  r1 |
  aes2. aes8 g8 |
  aes2. aes8 g8 |
  aes4. bes8 bes4. des'8 |
  des'2. ces'8 aes8 |
  bes2 r2 |
  r1 |
  r1 |
}

\score {
  \new StaffGroup <<
    \new Staff \with { instrumentName = "S" } << \global \soprano >>
    \new Staff \with { instrumentName = "A" } << \global \alto >>
    \new Staff \with { instrumentName = "T" } << \global \clef "treble_8" \tenor >>
    \new Staff \with { instrumentName = "B" } << \global \clef bass \bass >>
  >>
  \layout { }
  \midi { \tempo 2 = 46 }
}
