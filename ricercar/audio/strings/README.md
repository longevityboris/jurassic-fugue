...
| instruments | `iowa_build.py`: one string per note, trim, 48 kHz, grain-spliced 10 s sustain, attack pitch drift flattened (`attack_tune.py`), K-weighted level calibration, SFZ | every sample carries its attack analysis; SFZ regenerated when the builder or the tuning corrections are newer |
| tuning | closed loop: `verify_tuning.py` measures every key x layer through sfizz (YIN), `iowa_build.py --retune` folds the errors into `tuning_corrections.json` | every key x layer within 15 c (steady), every key x layer x stroke within 30 c 40-200 ms after the note-on (attack); stamped in `verified.sha256` so an unchanged install is not re-measured |
...
./render_demos.sh    # the old fugue (fugue-jp/fugue.ly, demo/fugue_plan.json), the
                     # contrabass-doubling variant, VPO3 comparison, dynamics proof
```
...
|---|---|---|
| `--wet` | -4 | reverb energy relative to the dry sound, both summed over the two channels; -4 gives C80 +8.4 dB (the same default value as the piano renderer's `--wet-db`). The report prints C80 |
...
mf 71-104, ff from 110), switches with hysteresis (up at 70 / 109, down at 66 /
...
the mirrored response) is normalised to unit energy over both channels, so
`--wet` is exactly reverb re dry. The measured file is 1.44 s long and faded at
...
| steady pitch | within 15 c after the closed loop | 15-18 c spread from vibrato, several keys 15-18 c off |
...
| | pp | mf | ff |
|---|---|---|---|
| Violin I | -37.9 dB, 764 Hz | -30.7 dB, 1387 Hz | -23.1 dB, 1621 Hz |
| Violin II | -38.7 dB, 814 Hz | -30.7 dB, 1180 Hz | -24.5 dB, 1415 Hz |
| Viola | -39.5 dB, 711 Hz | -30.5 dB, 720 Hz | -24.2 dB, 824 Hz |
| Cello | -38.5 dB, 295 Hz | -29.6 dB, 335 Hz | -22.1 dB, 405 Hz |
...
(detectors validated on planted events). C80 +8.4 dB; in 16th runs the new
note's pitch dominates in 78 % of pairs (93 % fully dry). The tail reaches -60 dB
...
| `verify_tuning.py`, `verify_dynamics.py`, `measure_dynamics.py`, `make_test_midi.py`, `qa_render.py` | verification and the dynamics proof |
